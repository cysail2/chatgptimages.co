...
import { copyShareLink, shareToSocial } from '@/library/lib/share/share-utils';
import { useToast } from '@/library/ui/toast-provider';
import { downloadMediaWithCors } from './utils';
import { UnifiedWorkItem } from './normalization';

export interface WorkActionsToolbarProps {
...
export function WorkActionsToolbar({
    item,
    onDeleteClick,
    showDelete = true,
    className = "",
}: WorkActionsToolbarProps) {
    ...
    const handleDownload = () => {
        // Use mediaUrl for download. For images it is the image url, for video it is the video url.
        // If audio, it is audio url.
        const url_to_download = item.mediaUrl;

        let filename = `media-${item.id}`;
        if (item.workType === 'video') filename += '.mp4';
        else if (item.workType === 'audio') filename += '.mp3';
        else filename += '.png'; // Default to png or derive from url

        downloadMediaWithCors(
            url_to_download,
            filename,
            (val) => setIsDownloading(!!val),
